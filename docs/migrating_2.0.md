...
- **[`navis.fix_mesh`][]`(fill_holes=True)` fills holes now.** It called `trimesh.Trimesh.fill_holes`, which - in every version {{ navis }} supports, 3.8 through 4.12 - closes single triangle and quad holes and skips everything larger without a word. On a 1M-face mesh cut in half that is 22 of 1030 openings; on the example neuron pruned at 5 µm it is 27% of the cap faces needed. It now goes through the same machinery as [`fill_holes`][navis.fill_holes] above, which is both complete and ~30x faster (278 ms :octicons-arrow-right-24: 9 ms on that mesh). [`Volume.validate`][navis.Volume.validate] gets the same treatment, via a new [`Volume.fill_holes`][navis.Volume.fill_holes] that overrides the trimesh method of that name.

    Two consequences worth knowing. **Outputs move**: a mesh that came back open now comes back closed, so volumes drop and `is_watertight` flips. And the fill now runs **after** the merge/dedupe/degenerate step rather than before it, because a boundary is a topological thing: an unmerged duplicate vertex invents openings that are not there, and a degenerate face sitting on a boundary edge hides one that is. What is left open afterwards is the non-manifold junctions the mesh was already broken at, which no amount of capping can close.

...
- **new [`navis.fill_holes`][] closes the holes in a [`Mesh`][navis.Mesh]** - the openings it was cut with, and any it came with. Cutting a mesh (via [`navis.prune_twigs`][], [`navis.prune_by_strahler`][], [`navis.subset_neuron`][], ...) drops every face that loses a corner, which used to leave each severed twig standing open. `fill_holes` triangulates those cross-sections shut:
    ...
    Openings are ear-clipped in their own plane rather than filled with a triangle fan, which matters because roughly a third of the cross-sections a prune leaves behind are not convex - a fan spills outside them. This needs the new `mapbox_earcut` dependency; without it the caps fall back to fans, which are worse but still close the hole and still wind correctly. No vertices are ever added, only faces, so vertex indices - and with them connectors, [`extra_edges`][navis.Mesh.extra_edges] and any tracked provenance - keep meaning what they meant.

    [`navis.subset_neuron`][] also takes a `cap_holes=True`, which closes only the openings that call itself made and leaves pre-existing ones alone. It is much the cheaper of the two - it only inspects the collar of faces around the cut, where `fill_holes` has to group the edges of the whole mesh (43 ms against 352 ms on a 2.2M-face mesh) - but it is off by default, so nothing changes for existing code unless you ask.
...
- **[`navis.simplify_mesh`][] left connectors pointing at vertices it had just deleted.** Decimation replaces a mesh's vertices wholesale, so on the example neuron simplifying to 20% left 83% of the connectors naming a vertex index that no longer existed - and the rest naming whatever had come to sit at that index. Connectors are now moved onto the new vertex closest to where they were (a median of 42 units on a neuron 24,000 units across). **Extra edges** survive it too, where they used to be dropped outright with a warning: a bridge names a place on the surface just like a connector does, so it follows its two endpoints (13 fragment-bridging edges on the example neuron come out as 12, having moved a median of 32 units - the one that goes is the one whose ends decimation merged into a single vertex, which is no longer an edge at all). Per-vertex data attached to a mesh is dropped rather than moved, since decimation invents vertex positions rather than choosing among the old ones - there is no correspondence that would make carrying it true.

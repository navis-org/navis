...
- a distance cap is called `max_dist` everywhere (was `limit_dist`, `limit` or `dist`), `min_size` always counts elements and `min_length` is always a distance (was `size`/`min_size` in [`prune_twigs`][navis.prune_twigs]/[`split_neurites`][navis.split_neurites]) - and all of them accept unit strings. Every old spelling still works with a `DeprecationWarning` (including `drop_fluff(keep_size=)`, `heal_*(drop_disc=)` and `smooth_mesh(L=)`); passing both is a `TypeError`
- connected components have one name, one shape and one home: new [`connected_components`][navis.connected_components] plus `.connected_components()`/`.n_components` on every neuron type. `break_fragments` :material-arrow-right-thin: [`split_components`][navis.split_components], `split_into_fragments` :material-arrow-right-thin: [`split_neurites`][navis.split_neurites], `Skeleton.subtrees`/`.n_trees`/`.is_tree` :material-arrow-right-thin: `.connected_components()`/`.n_components`/[`.is_acyclic`][navis.Skeleton.is_acyclic]
- every morphology function that takes a single neuron now has a method, and the methods drop the type suffix (e.g. `Skeleton.despike()`, `Mesh.heal()`, `Neuron.subset()`)
- removed: the `rpy2`-based R interface (use the new `read_rds`/`write_rds` & co.), the Cytoscape interface, the `vispy` backend (and with it `navis.Viewer`) and the `pyfqmr`/`open3d`/`pymeshlab`/Blender mesh backends
- [`find_soma`][navis.find_soma] now returns a single node ID (or `None`) instead of an array of candidates
- alignment runs on [rcpd](https://github.com/schlegelp/rcpd) instead of the unmaintained `pycpd`: 20-28x faster, four long-standing bugs fixed, and different registration objects (new optional dependency, `pip install navis[cpd]`)
...
- new [`Neuron.attach`][navis.BaseNeuron.attach] and [`attach_link`][navis.BaseNeuron.attach_link]: your own per-element data, carried through subsetting, masking and - where the function can say what happened - rebuilds (see new tutorial)
- new [`plot_collage`][navis.plot_collage]: a few hundred neurons packed onto one page, with either renderer (see new tutorial)
- new [`plot3d`][navis.plot3d]`(..., snapshot=True)`: an octarine render placed on a matplotlib axes, in the neurons' own coordinates
- new [`nblast_knn`][navis.nblast_knn]: the `k` nearest neighbours of every neuron without ever building the score matrix (164k neurons: a 26 MB k-NN graph instead of a 107 GB matrix)
- new for meshes: [`heal_mesh`][navis.heal_mesh], [`fill_holes`][navis.fill_holes], [`find_soma_mesh`][navis.find_soma_mesh] and [`Mesh.extra_edges`][navis.Mesh.extra_edges] (connectivity the surface itself does not have)
- new [`Voxels`][navis.Voxels] toolkit - `dilate`/`erode`/`thin`/`fill_cavities`, set algebra, `surface_area`/`centroid`/`distance_transform`/`iou`/`dice`, `.mesh()`/`.skeletonize()` - all working straight off the sparse voxels; [`skeletonize`][navis.skeletonize] now accepts `Voxels` too
- new functions for the angles in a skeleton: [`branch_angles`][navis.branch_angles], [`path_angles`][navis.path_angles], [`root_angles`][navis.root_angles] and [`soma_exit_angles`][navis.soma_exit_angles]
- new [`cast_neuron`][navis.cast_neuron], [`graph.geodesic_clusters`][navis.graph.geodesic_clusters], [`transforms.SimilarityTransform`][navis.transforms.SimilarityTransform] and [`patch_caveclient`][navis.patch_caveclient]
- new `navis.ml` module: normalization, augmentation, sampling and chunking to prepare neurons as machine-learning inputs (three new tutorials)
- new R interop that needs neither R nor `rpy2`: [`read_rds`][navis.read_rds]/[`write_rds`][navis.write_rds] and [`read_rda`][navis.read_rda]/[`write_rda`][navis.write_rda] (see new natverse tutorial)
...
- [`simplify_mesh`][navis.simplify_mesh] and [`smooth_mesh`][navis.smooth_mesh] run on fastcore: `simplify_mesh` returns a vertex map, so connectors, extra edges, attached data and a mesh's skeleton survive decimation; `smooth_mesh` is ~50x faster and defaults to Taubin's λ|μ filter
- [`fix_mesh`][navis.fix_mesh]`(fill_holes=True)`, the new [`Volume.fill_holes`][navis.Volume.fill_holes] and [`Volume.validate`][navis.Volume.validate] cap their openings with the same fastcore machinery as [`fill_holes`][navis.fill_holes] rather than trimesh's, which in every version {{ navis }} supports closes single triangle and quad holes and nothing else. A 1M-face mesh cut in half has 1030 openings, of which trimesh's could do 22 (278ms :octicons-arrow-right-24: 9ms); the caps are ear-clipped rather than fanned, so they no longer fold over themselves on a non-convex opening. In `fix_mesh` the fill also runs *after* the clean-up now, so a duplicate vertex or a degenerate face can no longer invent an opening that is not there or hide one that is
- a [`Mesh`][navis.Mesh] now keeps its skeleton - and its node IDs - through a subset or a mask instead of throwing it away and re-skeletonizing
- [`plot2d`][navis.plot2d]`(method="2d")` has a new renderer: meshes are drawn as surfaces (back-face culling, painter's algorithm) so they occlude themselves, `mesh_shade` lights them, and new `depth_sort`/`halo` arguments stand in for the depth buffer matplotlib does not have
- connector plotting has been overhauled: one artist per neuron in a fixed shuffled order, new `cn_color_by`/`cn_palette`/`cn_legend`, and `cn_layout={"display": "lines"}` finally honoured by matplotlib (see new tutorial)
- the plotting backends assemble their lines in one pass (100 neurons: 120ms :octicons-arrow-right-24: 27ms; k3d's coloured path 672ms :octicons-arrow-right-24: 38ms) and share the code that resolves connectors, colors, somata and `shade_by`
- the plotting tutorials have been restructured into *General*, *2D plotting*, *3D plotting*, *Other plots* and *Examples* (old `generated/gallery/1_plotting/` URLs have moved)
- `parallel=True` no longer fights the threads underneath it: each worker is told how many it may use (new `inner_max_num_threads`, plus [`set_num_threads`][navis.set_num_threads] for the other direction), and `n_cores` follows the cores this *process* may use
- CMTK and elastix point transforms run in-process via fastcore - no binaries, no temporary files, and `xform_brain` is 4-30x faster end-to-end; the landmark transforms (TPS, moving least squares) gained a fastcore backend and no longer scale their memory with the landmark count
- [`Voxels`][navis.Voxels] avoid the dense grid wherever possible (requires `sparse-cubes`), and materialising one larger than `navis.config.max_grid_size` now raises instead of being OOM-killed
- [`make_dotprops`][navis.make_dotprops] is ~12x faster and [`nbl.extract_matches`][navis.nbl.extract_matches] 1.3-118x (it also gained a `max_matches` guard)
- the remote data-source interfaces share their plumbing: results come back in the order you asked for them, failures go through `errors` rather than `print`, and importing an interface no longer requires its dependencies or a network connection
...
- rebuilding a neuron left connectors, tags, the soma and anything you had attached pointing at elements that no longer exist: [`downsample_neuron`][navis.downsample_neuron], [`insert_nodes`][navis.insert_nodes], [`prune_by_strahler`][navis.prune_by_strahler], [`simplify_mesh`][navis.simplify_mesh] and `merge_duplicate_nodes` were all affected
- [`ivscc_features`][navis.ivscc_features] had six features that were quietly wrong (`max_euclidean_distance`, `calculate_number_of_stems`, `max_branch_order`, `mean_contraction`, `exit_distance`/`exit_theta`, and everything that missed a compartment's most proximal fork)
- [`sholl_analysis`][navis.sholl_analysis] was broken for most of its `center` options - including the default - and returned wrong numbers without raising when given a numpy integer node ID
- [`make_dotprops`][navis.make_dotprops] silently produced wrong tangent vectors for point clouds with duplicate coordinates, and [`Dotprops.recalculate_tangents`][navis.Dotprops.recalculate_tangents] returned `NaN` alphas for them
- [`nblast`][navis.nblast] with `scores="both"` crashed on every multi-core run; [`nbl.extract_matches`][navis.nbl.extract_matches] ranked `NaN` as the best possible score and listed `percentage` matches worst-first for distance matrices
- [`write_parquet`][navis.write_parquet] silently dropped connectors (they now go into a sidecar file) and the `label` column, and [`read_parquet`][navis.read_parquet] returned a `NeuronList` for single-neuron files
- reading with a list `limit` (e.g. `limit=["neuron.swc"]`) returned nothing at all for most sources
- [`read_precomputed`][navis.read_precomputed] silently shattered skeletons whose edges are not in the column order it expected (the male CNS skeletons came out with 378 roots where the graph has 4)
- [`collapse_nodes`][navis.collapse_nodes] raised `MemoryError` on real (i.e. large) node IDs and never updated the vertex map it meant to
- `geodesic_matrix(directed=True)` reported a coincident child as reachable from its parent, and weighted segment lengths were one edge too long
- a batch of [`Voxels`][navis.Voxels] bugs, most of them on the sparse backing: `threshold()`, `normalize()`, `convert_units()`, `.volume`, `.bbox`, `deepcopy()`, `flip()` and empty neurons
- `parallel=True` could hang forever on macOS once the process had done any real work, printed leaked-semaphore warnings, and mislabelled dataframes with the wrong neuron under `omit_failures=True`
- [`memory_usage`][navis.BaseNeuron.memory_usage] did not count data held inside something else and raised with `estimate=True` on any neuron with connectors - which is why a [`NeuronList`][navis.NeuronList] reported `0.0B`

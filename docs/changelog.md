...
- **the `pathos` backend printed `resource_tracker: There appear to be 1 leaked semaphore objects to clean up` once per worker.** Building a progress bar makes `tqdm` allocate a cross-process lock - a named semaphore - and `pathos`' workers come from `multiprocess`, which carries its own resource tracker, so each worker spawned a second, stdlib one that then reported the lock as leaked on the way out. Workers now say up front that their bars aren't shared across processes, so no such lock is allocated. Only `pathos` was affected.

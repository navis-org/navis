...
- **[`navis.simplify_mesh`][] runs on [navis-fastcore](https://github.com/schlegelp/fastcore-rs) and the other four backends are gone.** `pyfqmr`, `open3d`, `pymeshlab` and Blender 3D all did the same job, needed installing separately, disagreed about what `F` meant, and - the point - none of them could say what had become of a vertex. fastcore's implementation is the same Garland-Heckbert quadric decimation `pyfqmr` runs, and it returns a **vertex map**: for each old vertex, the new one it was merged into. That is what everything below is built on.
    ...
    Two smaller consequences. `navis.meshes.available_backends` is now `navis.meshes.smoothing_backends`, which is all it was ever used for; and [`navis.smooth_mesh`][] with `backend="auto"` was **silently doing nothing** on any machine with `pyfqmr` installed, since `pyfqmr` came first in that list and smoothing has no `pyfqmr` branch. It now picks `open3d`, else `trimesh`. Blender is deliberately last, behind a backend that is always there: it is reached by writing the mesh out to STL and reading it back, which reorders the vertices - fine on its own, wrong for a function whose whole contract is that vertex indices survive. It is still selectable by name, and its script no longer produces an *empty* mesh on Blender 4.0+, which renamed the STL operators it used.

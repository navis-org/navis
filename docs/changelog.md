...
- **[`navis.find_soma`][navis.find_soma] now returns a single node ID (or `None`) instead of an array of candidates.** It used to hand back every node passing the radius/label filter and leave the choice to the caller, which meant a thick primary neurite could be returned alongside - or instead of - the actual soma. Candidates are now clustered spatially around the fattest one (within `dist_factor` times its radius, new argument) and the fattest node of that cluster wins; the label-only path takes the most central node of the largest connected label component. Nodes whose radius is missing (`NaN` or `<= 0`, as `guess_radius` writes) are no longer treated as candidates, and a radius criterion that matches nothing usable no longer suppresses the label criterion. Code doing `find_soma(n)[0]` or `len(find_soma(n))` needs updating.

...
CAVE_DATASTACKS = {
    'h01_c3_flat': 'h01_c3_flat',
}

SEG_URLS = {
    'h01_c3_flat': 'graphene://https://local.brain-wire-test.org/segmentation/table/h01_full0_v2'
}

class Authentication:
    def __init__(self):
        self.url = "https://global.brain-wire-test.org/"
        self.auth = caveclient.auth.AuthClient(server_address=self.url)
    
    def setup_token(self, make_new=True):
        self.auth.setup_token(make_new)
    
    def save_token(self, token, overwrite):
        self.auth.save_token(token, overwrite)
        

@lru_cache(None)
def get_cave_client(datastack='h01_c3_flat'):
...
def get_somas(root_ids, table='nucleus', datastack='h01_c3_flat'):
